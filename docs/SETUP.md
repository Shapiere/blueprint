...
- `~/.pi/agent/extensions/runtime-orchestrator.ts` — Runtime Abstraction Layer (RAL) v1 (Phases 1–6): `session_start` project topology detection (Node/TS, Python, Rust, Go, Roblox, Generic); 9router health probe with auto-start fallback; `before_agent_start` workspace context injection + per-turn capability scoping + orchestration governance contract; complexity-aware workflow enforcement (strategy caps DIRECT=1/LIGHT≤3/FULL≤8, HEAVY user approval via interactive select); dynamic model catalog bridge via Pi-native `refreshModels`; integrated `/model` flow (model → reasoning profile → reasoning level → confirm; capability-aware); model silent-switch guard; `/doctor`, `/reasoning`, `/sync` commands. Source: `capabilities/extensions/runtime-orchestrator.ts`. Type-check PASS 2026-08-23 (TS 7.0.2). Decisions D33–D38.
