...
## Runtime Abstraction Layer v1 (implemented 2026-08-22/23, D33–D38)
...
- `before_agent_start` hook (every turn): inject minimal workspace context string (~30–50 tokens), perform per-turn capability scoping — rebuild the `<available_skills>` section so only skills relevant to the detected project profile remain visible (D35) — and append the orchestration governance contract + active reasoning profile/level (D37). Fail-open on any scoping failure.
- `/doctor` command: diagnostic scan of Pi runtime, 9router, MCP configuration, permission system, core extensions, sync status, model catalog drift, and capability scoping state.
- `/sync` command (D34): deterministic one-way asset deployment from Blueprint (`G:/pisetup`) to runtime (`~/.pi/agent/`). SHA256 drift detection; conflicts block silent overwrites unless `--force`; protected files never touched. Allowlist includes `capabilities/scopes.json`.
- Dynamic model catalog bridge (Phase 4, D36): registers provider `"9router"` via Pi-native `pi.registerProvider` with `refreshModels` fetching `/v1/models` — `/model` reflects the live router catalog. `models.json`/`auth.json` never read/written by this path.
- Complexity-aware orchestration (Phase 5, D37): workflow scripts declare complexity (`DIRECT|LIGHT|FULL|HEAVY` + workstreams/parallelizable/risk); the extension intercepts the `workflow` tool call and enforces strategy caps (DIRECT=1, LIGHT≤3, FULL≤8); HEAVY requires explicit interactive approval; requested maxAgents are clamped to the approved ceiling. Model silent-switch guard blocks/prompts on explicit model overrides in scripts.
- Integrated `/model` reasoning flow (Phase 6, D38): after a model is selected via Pi's native selector, RAL presents Reasoning Profile (10 profiles; Vision gated on image input) then Reasoning Level (user-facing Off/Low/Medium/High/Ultra → canonical off/low/medium/high/xhigh) selectors, persists to `harness-reasoning.json`, and applies via native `pi.setThinkingLevel()` (clamps to model capabilities). Cancelling keeps prior configuration. `/reasoning` retained for CLI compatibility.
- Model Control Center (`/mcc`, D39): a re-enterable, grouped overview (GENERAL / PLANNING / EXECUTION / SPECIALIZED) of all 10 reasoning profiles with effective levels at a glance, plus the current model and an inline Select Model action (`ctx.modelRegistry` + `pi.setModel`). Selecting a profile opens a focused level editor (Off/Low/Medium/High/Ultra) persisting immediately per edit — cancel-safe. State model v2 in `harness-reasoning.json`: `activeProfile`/`activeLevel` + per-profile overrides, with legacy single-pair migration.
...
**What the RAL does NOT do:** task-aware activation beyond the complexity gate (deferred), bidirectional sync, MCP enable/disable automation, capability registry modification, permission override, durable skill-file mutation, `models.json` writes, polling loops, automatic model selection (user owns model choice).
...
### D53 addendum — final `/model` information architecture (2026-09-01)

The D50 three-region composition (NAVIGATION | MODELS | DETAIL with profiles in the nav) is retired. The final surface: a two-pane PROVIDERS | MODELS browser on top (provider selection scopes the model list; pane title mirrors scope — "9ROUTER MODELS" / "ALL MODELS"; model rows carry no provider prefix), a focus-following detail region below it (PROVIDER → "Configured / available to Pi" + "Connectivity: Unverified"; SELECTED MODEL → horizontal metadata `200k ctx · ● reasoning · ○ vision · 9router / bai` + `✓ Current Model`; PROFILE → level + state), a horizontally dense REASONING PROFILES region (all ten profiles as wrapped chips; › focus, ★ default, ● execution, capability-gated rows dimmed and skipped), and a contextual footer per focused region. ←→ switch regions; typing filters the model scope with a visible `> query` line; Esc closes (a non-empty search clears first). `ModelSurfaceState` persists focus, scope, filter and profile cursor across the loop; `modelSurfaceLoops` absorbs the `model_select` events fired by the surface's own `setModel` so no second loop stacks. D36/D42 catalog semantics, D37 guards, D44/D51 bridge unchanged.
