...
- Rationale: Separates four previously conflated dimensions — task complexity (orchestration need), model selection (user authority), reasoning level (depth per function), agent count (justified parallelism). Simple tasks stay simple under any effort level; expensive fan-out requires informed consent; the session model can never change silently.
