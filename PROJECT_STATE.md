...
- Continuous Evolution (2026-08-23): `/mcc` Model Control Center implemented (D39) — grouped multi-profile reasoning overview (GENERAL/PLANNING/EXECUTION/SPECIALIZED) with at-a-glance levels, per-profile editors with immediate persistence (cancel-safe), inline model selection, Vision capability gating, v2 state migration in `harness-reasoning.json`.
