...
export interface PermissionsService extends PermissionQuery {
  ...
  /**
   * Register a custom terminal prompt renderer that replaces the stock
   * PermissionPromptComponent UI at the LocalUserAuthorizer level.
   *
   * The package remains the sole authority: the renderer only owns
   * presentation and resolves the user's choice into a
   * PermissionPromptDecision; the package commits that decision via
   * GateRunner / SessionRules / audit exactly as it does for the stock prompt.
   *
   * The renderer is invoked only for interactive local sessions
   * (LocalUserAuthorizer, hasUI). ParentAuthorizer (subagent forward) and
   * DenyingAuthorizer (headless/no-authority) never use it.
   *
   * When no renderer is registered, the stock PermissionPromptComponent is
   * used, preserving package compatibility. Only one renderer may be
   * registered at a time — a second call throws. The returned disposer
   * unregisters the renderer.
   *
   * Register from a `permissions:ready` handler, mirroring
   * `registerAuthorizer`. The renderer is handed the same arguments the
   * stock LocalUserAuthorizer would have passed to
   * `requestPermissionDecision`: PromptPermissionDetails + view (mode/ui/
   * doublePressToConfirm) + title/message/options.
   *
   * @param renderer - The prompt renderer callback
   *                   (`(details, view, title, message, options) => Promise<PermissionPromptDecision>`)
   */
  registerPermissionPromptRenderer(
    renderer: (
      details: import("./authority/permission-prompter").PromptPermissionDetails,
      view: import("./authority/permission-prompt-component").PermissionPromptView,
      title: string,
      message: string,
      options?: import("./authority/permission-dialog").RequestPermissionOptions,
    ) => Promise<import("./authority/permission-dialog").PermissionPromptDecision>,
  ): () => void;
}
