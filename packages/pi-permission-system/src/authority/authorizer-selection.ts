  ...
  implements AskEscalator, AuthorizerSelectionLifecycle
{
  ...
  constructor(
    private readonly deps: AuthorizerSelectionDeps & {
      ...
      /** Optional prompt renderer registry — LocalUserAuthorizer checks it before stock. */
      promptRendererRegistry?: { get(): ((details: PromptPermissionDetails, view: PermissionPromptView, title: string, message: string, options?: RequestPermissionOptions) => Promise<PermissionPromptDecision>) | null };
    },
  ) {}
   ...
   */
  activate(ctx: ExtensionContext): void {
    const promptRendererRegistry = (this.deps as unknown as { promptRendererRegistry?: { get(): unknown } }).promptRendererRegistry as unknown as { get(): unknown } | undefined;
    this.terminal = selectAuthorizer(ctx, {
      ...(this.deps as unknown as AuthorizerSelectionDeps),
      getPromptRenderer: () => (promptRendererRegistry as unknown as { get(): unknown })?.get() as never,
    } as unknown as AuthorizerSelectionDeps);
  }
  /**
