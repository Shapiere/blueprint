...
export interface LocalUserAuthorizerDeps {
  ...
  /** Optional prompt renderer that replaces the stock PermissionPromptComponent when present. */
  getPromptRenderer?: () => ((details: PromptPermissionDetails, view: PermissionPromptView, title: string, message: string, options?: RequestPermissionOptions) => Promise<PermissionPromptDecision>) | null;
}
...
export class LocalUserAuthorizer implements TerminalAuthorizer {
  ...
  authorize(
    ...
    const view: PermissionPromptView = {
      mode: this.deps.mode,
      ui: this.deps.ui,
    ...
    const renderer = this.deps.getPromptRenderer?.();
    if (renderer) {
      return renderer(details, view, title, message, options);
    }
    return this.deps.requestPermissionDecision(view, title, message, options);
  }
}
