...
13. **D63 (2026-09-04):** `/model` provenance fix — the D51 host-bridge blob dispatched the extension `model` command on EVERY submitted line (typing "testing" + Enter opened the MCC). After any `pi update`, re-run `node capabilities/scripts/pi-model-bridge.mjs apply` — the script now migrates the legacy unguarded blob automatically. `/model` must open ONLY from explicit `/model` input; programmatic model events never open it. The D62 report's "~20 s startup auto-open" was this same defect (input during the boot window), not a catalog/restore event.
